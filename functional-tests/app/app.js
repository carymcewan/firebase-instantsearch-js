...
search.addWidget(instantsearch.widgets.hitsPerPageSelector({
  ...
  items: [{ value: 6, label: '6 per page' }, { value: 12, label: '12 per page' }, { value: 24, label: '24 per page' }],
  cssClasses: {
    select: 'form-control'
  }
}));
...
search.addWidget(instantsearch.widgets.priceRanges({
 ...
 */
function recorder(algoliasearch, app, key) {
  // eslint-disable-line
  var client = algoliasearch(app, key);
  var originalSearchFn = client.search;
  client.search = function (queries, cb) {
    var maybeResults = originalSearchFn.call(client, queries);
    maybeResults.then(function (res) {
