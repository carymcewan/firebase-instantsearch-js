...
// Read CLI flags
var forceBeta = process.argv.includes('--beta');

var _require = require('./conventionalChangelog.js'),
...
var _shell$exec = shell.exec('$(npm owner add `npm whoami`)', {
  silent: true
}),
    isNotOwner = _shell$exec.code;
...
var uncommitedChanges = shell.exec('git status --porcelain').toString().trim();
...
var currentBranch = shell.exec('git rev-parse --abbrev-ref HEAD').toString().trim();
...
var strategy = currentBranch !== 'develop' || forceBeta ? 'beta' : 'stable';
...
inquirer.prompt([{
  type: 'confirm',
  name: 'confirm',
  message: colors.yellow.underline('You are on "' + currentBranch + '" branch. Are you sure you want to release a ' + (strategy === 'stable' ? 'new stable' : 'beta') + ' version?'),
  default: false
}]).then(function (_ref) {
  ...
  shell.echo(colors.blue('\n      - Current version is "' + currentVersion + '"\n      - Changelog will be generated only if a fix/feat/performance/breaking is found in git log\n      - You must choose a new ve.rs.ion (semver) ' + (strategy === 'beta' ? 'with -beta.x suffix' : '') + '\n    '));
  ...
  inquirer.prompt([{
    type: 'input',
    name: 'newVersion',
    message: colors.blue.underline(strategy === 'stable' ? 'Please type the new chosen version' : 'Please type the new chosen version (with -beta.x suffix)'),
    validate: function validate(newVersion) {
      return strategy === 'stable' || newVersion && /-beta\.\d{1,}$/.test(newVersion) || 'You must include `-beta.x` suffix in the version number';
    }
  }]).then(function (_ref2) {
    var newVersion = _ref2.newVersion;

    // bump new version
    shell.echo(colors.blue('Bump version to "' + newVersion + '"'));
    shell.exec('VERSION=' + newVersion + ' babel-node ./scripts/bump-package-version.js');
    ...
    // git add and tag
    var commitMessage = 'v' + newVersion + '\n\n' + changelog;
    shell.exec('git add src/lib/version.js yarn.lock package.json CHANGELOG.md README.md CONTRIBUTING.md');
    shell.exec('git commit -m "' + commitMessage + '"');
    shell.exec('git tag "v' + newVersion + '"');

    shell.echo(colors.yellow.underline('Almost done, check everything in another terminal tab'));

    inquirer.prompt([{
      type: 'confirm',
      name: 'publishNpm',
      message: colors.yellow.underline('Is everything OK? Continue to push to github and publish the package?')
    }]).then(function (_ref3) {
      var publishNpm = _ref3.publishNpm;
